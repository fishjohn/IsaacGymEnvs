...
from isaacgymenvs.utils.torch_jit_utils import to_torch, get_axis_params, torch_rand_float, quat_rotate, \
    quat_rotate_inverse
...
class Biped(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        asset_options.collapse_fixed_joints = True  # TODO: Check this param
        ...
        body_names = self.gym.get_asset_rigid_body_names(biped_asset)
        self.dof_names = self.gym.get_asset_dof_names(biped_asset)
        extremity_name = "foot"
        ...
        for i in range(self.num_envs):
            ...
            biped_handle = self.gym.create_actor(env_ptr, biped_asset, start_pose, "biped", i, 1, 0)
            self.gym.set_actor_dof_properties(env_ptr, biped_handle, dof_props)
            self.gym.enable_actor_dof_force_sensors(env_ptr, biped_handle)
            self.envs.append(env_ptr)
            self.biped_handles.append(biped_handle)

        for i in range(len(feet_names)):
            self.feet_indices[i] = self.gym.find_actor_rigid_body_handle(self.envs[0], self.biped_handles[0],
                                                                         feet_names[i])
        for i in range(len(knee_names)):
            self.knee_indices[i] = self.gym.find_actor_rigid_body_handle(self.envs[0], self.biped_handles[0],
                                                                         knee_names[i])

        self.base_index = self.gym.find_actor_rigid_body_handle(self.envs[0], self.biped_handles[0], "base_Link")
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:] = compute_biped_reward(
            # tensors
            self.root_states,
            self.commands,
            self.torques,
    ...
    def compute_observations(self):
        ...
        self.obs_buf[:] = compute_biped_observations(  # tensors
            self.root_states,
            self.commands,
            self.dof_pos,
